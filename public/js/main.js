...
async function onclick(e){
    ...
    if(e.target && e.target.classList.contains("del-btn")){
        ...
        try{
            await axios.get(`http://localhost:8080/user/tracker/delete/${dID}`);
            refresh();
        }catch(err){
            console.log(err);
        }
        ...
        try{
           const response=  await axios.get(`http://localhost:8080/user/tracker/edit/${eID}`);
          const {amount} = response.data;
          useramount.value = amount;
          await axios.get(`http://localhost:8080/user/tracker/delete/${eID}`);
          refresh();
        }catch(err){
            console.log(err);
        }
...
async function refresh(){
    try {
        const response = await axios.get('http://localhost:8080/user/tracker/data');
        showOutput(response);
